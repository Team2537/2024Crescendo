...
//    private var command: Optional<Command> = Optional.empty()
     ...
     */
    fun build(isRed: Boolean): Command {
...
//        }
        ...
        pathList.forEachIndexed() { index, path ->
            val parallel: Command = parallelEventMap.getOrDefault(index, Supplier { InstantCommand() }).get()

            auto.addCommands(
                getPath(path, isRed, swerve, parallel),
                sequentialEventMap.getOrDefault(index, Supplier { InstantCommand() }).get()
            )
        }
